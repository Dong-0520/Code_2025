...
ref = QuadRefElemFromDiagETriangle(order, 2 * order, true)
...
grid = read_mesh(joinpath(@__DIR__, "mesh/BurgersMeshSlab_05tB_Quad.msh"), ref_elems_data, identity)
grid = read_mesh(joinpath(@__DIR__, "mesh/BurgersMeshSlab_01_Quad.msh"), ref_elems_data, identity)
grid = read_mesh(joinpath(@__DIR__, "mesh/BurgersMeshSlab_01_Quad_nonSquare.msh"), ref_elems_data, identity)
...
@time all_U, diffs = space_time_solve(U0 * 0, p1, pseudo_dt = 0.001, num_of_pseudo_time_step = 10000, diss = true, test = true)
...
@time all_U2, diffs2 = space_time_solve(u1, p2, pseudo_dt = 0.001, num_of_pseudo_time_step = 20000, diss = true, test = true)
...
@time all_U3, diffs3 = space_time_solve(u2, p3, pseudo_dt = 0.001, num_of_pseudo_time_step = 1000, diss = true, test = true)
...
# 求解问题
sol_slab3 = solve(prob_with_jac_diss, NewtonRaphson(linsolve = KrylovJL_GMRES()),show_trace = Val(true), maxiters = 1000)
observe_numerical_sol(grid, sol_slab3.u, zlimit, show_true_sol = false)
...
p3 = get_set_up(grid, u = sol_slab2.u, slab = 3)
IC3 = p3[end]
...
@time all_U3, diffs3 = space_time_solve(u2, p3, pseudo_dt = 0.0005, num_of_pseudo_time_step = 1000, diss = true, test = true)
gif(observe_iteration(grid, all_U3[1:10:end, :, :], zlimit, show_true_sol = false), joinpath(@__DIR__, "numerical_test_discts.gif"), fps=5)
...
# 求解问题
sol_slab3_test1 = solve(prob_with_jac_diss, NewtonRaphson(linsolve = KrylovJL_GMRES()),show_trace = Val(true), maxiters = 1000)
...
#---------------------- solver only to solve big slab ---------------------- 
p1 = get_set_up(grid, slab = 1)
IC1 = p1[end]

U0 = initial_guess(grid)
@time all_U, diffs = space_time_solve(U0 * 0, p1, pseudo_dt = 0.0005, num_of_pseudo_time_step = 1000, diss = true, test = true)
gif(observe_iteration(grid, all_U[1:5:end], zlimit, show_true_sol = false), joinpath(@__DIR__, "numerical_test_discts.gif"), fps=5)
u1 = all_U[500]
observe_numerical_sol(grid, u1, zlimit, show_true_sol = false)
...
# 求解问题
sol = solve(NonlinearProblem(nonlinearFunction_with_jac_diss, U0_for_solver * 0, p1; abstol=1e-10, reltol=1e-10), NewtonRaphson(linsolve = KrylovJL_GMRES()),show_trace = Val(true), maxiters = 1000)
observe_numerical_sol(grid, sol.u, zlimit, show_true_sol = false)

# 求解问题
sol1 = solve(prob_with_jac_diss, NewtonRaphson(linsolve = KrylovJL_GMRES()),show_trace = Val(true), maxiters = 1000)
observe_numerical_sol(grid, sol1.u, [-1.5, 1.5], show_true_sol = false)

@time sol2 = solve(prob_with_jac_diss, NewtonRaphson(),show_trace = Val(true), maxiters = 1000)
observe_numerical_sol(grid, sol2.u, [-1.5, 1.5], show_true_sol = false)

@time sol3 = solve(prob_with_jac_diss, LevenbergMarquardt(), show_trace = Val(true),  maxiters = 1000)
observe_numerical_sol(grid, sol3.u, [-1.5, 1.5], show_true_sol = false)

@time sol4 = solve(prob_with_jac_diss, TrustRegion(), show_trace = Val(true),  maxiters = 1000)
observe_numerical_sol(grid, sol4.u, [-1.5, 1.5], show_true_sol = false)

@time sol5 = solve(prob_with_jac_diss, PseudoTransient(), show_trace = Val(true))
observe_numerical_sol(grid, sol5.u, [-1.5, 1.5], show_true_sol = false)

@time sol6 = solve(NonlinearProblem(nonlinearFunction_with_jac_diss, U0_for_solver * 0, p2; abstol=1e-10, reltol=1e-10), PseudoTransient(), show_trace = Val(true))
observe_numerical_sol(grid, sol6.u, [-1.5, 1.5], show_true_sol = false)

#---------------------- solver only to solve big slab2 ---------------------- #
u1 = sol3.u
p2 = get_set_up(grid, u = u1, slab = 2)
IC2 = p2[end]
...
@time all_U2, diffs2 = space_time_solve(u1, p2, pseudo_dt = 0.001, num_of_pseudo_time_step = 1000, diss = true, test = true)
gif(observe_iteration(grid, all_U2[1:5:end, :, :], zlimit, show_true_sol = false), joinpath(@__DIR__, "numerical_test_discts.gif"), fps=5)
# u2 = all_U2[argmin(diffs2)]
u2 = all_U2[200]
observe_numerical_sol(grid, u2, zlimit, show_true_sol = false)

# to solve slab 2 using Julia Built in solver
U0_for_solver = u2
...
# 求解问题
sol2_slab2 = solve(prob_with_jac_diss, NewtonRaphson(),show_trace = Val(true), maxiters = 1000)
observe_numerical_sol(grid, sol2_slab2.u, zlimit, show_true_sol = false)

sol3_slab2 = solve(prob_with_jac_diss, LevenbergMarquardt(), show_trace = Val(true),  maxiters = 1000)
observe_numerical_sol(grid, sol3_slab2.u, zlimit, show_true_sol = false)

sol4_slab2 = solve(prob_with_jac_diss, TrustRegion(), show_trace = Val(true),  maxiters = 1000)
observe_numerical_sol(grid, sol4_slab2.u, zlimit, show_true_sol = false)
