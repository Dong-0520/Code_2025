...
grid = read_mesh(joinpath(@__DIR__, "Burgers_SIAM_const_05.msh"), ref_elems_data, Base.identity)

# quadrilateral mesh
ref = QuadRefElemGL_Hicken(order, boundary_nodes = true)
ref_elems_data = Dict{String, SBPLite.RefElemData}("Quadrilateral 4" => ref)
grid = read_mesh(joinpath(@__DIR__, "Quad_grid.msh"), ref_elems_data, Base.identity)


# @save joinpath(@__DIR__, "Burgers_SIAM_const_mesh.jld2") grid
# @load joinpath(@__DIR__, "Burgers_SIAM_const_mesh.jld2") grid
...
all_U, diffs = space_time_solve(U0, p1, pseudo_dt = 8.0, num_of_pseudo_time_step = 1000)
plot(diffs)
