...
function plot_numerical_sol_at_final_time_perSlab(grid, u; color = :blue, title = 0.0, ylim = [-1.1, 1.1])
    ...
    xR = []
    uR = []
    for (cell_id, _) in collect(get_face_set(grid, "TOP_INFLOW_1"))
        push!(xL, x_coords[cell_id])
        push!(uL, num_sol[cell_id])
    end
    for (cell_id, _) in collect(get_face_set(grid, "TOP_INFLOW_2"))
        push!(xR, x_coords[cell_id])
        push!(uR, num_sol[cell_id])
    end
    ...
    xR_flat = vcat(xR...)
    uR_flat = vcat(uR...)
    xL_sorted = sort(xL_flat)
    uL_sorted = uL_flat[sortperm(xL_flat)]
    xR_sorted = sort(xR_flat)
    uR_sorted = uR_flat[sortperm(xR_flat)]

    # 画连续曲线
    plot!(p, xL_sorted, uL_sorted, label="", color=color)
    plot!(p, xR_sorted, uR_sorted, label="", color=color)
    scatter!(p, xL_sorted, uL_sorted, shape=:utriangle, markersize=3, color=:red)
    scatter!(p, xR_sorted, uR_sorted, shape=:utriangle, markersize=3, color=:red)
end
...
function scatter_gif_numerical_sol_for_all_slabs(grid::Grid, all_ui::Vector{Matrix{Float64}}, dt::Float64; 
    ...
    anim = @animate for i in 0:length(all_ui)
        if i == 0
...
function plot_gif_numerical_sol_for_all_slabs(grid::Grid, all_ui::Vector{Matrix{Float64}}, dt::Float64; 
    ...
    top_inflow1 = collect(get_face_set(grid, "TOP_INFLOW_1"))
    top_inflow2 = collect(get_face_set(grid, "TOP_INFLOW_2"))
    x_coords = get_xCoords_for_plotting_sol(grid)
    anim = @animate for i in 0:length(all_ui)
        xL = []
        uL = []
        ...
        if i == 0
            ...
            for (cell_id, face_id) in bottom_inflow1
                ref = grid.cells[cell_id].ref_data[]
                R = ref.R[face_id]
                push!(xL, map(x -> x[1], R * grid.xyz_q[cell_id]))
                push!(uL, [analytic_u(x) for x in R * grid.xyz_q[cell_id]])
            end
            for (cell_id, face_id) in bottom_inflow2
                ref = grid.cells[cell_id].ref_data[]
                R = ref.R[face_id]
                push!(xR, map(x -> x[1], R * grid.xyz_q[cell_id]))
                push!(uR, [analytic_u(x) for x in R * grid.xyz_q[cell_id]])
            end
            ...
            for (cell_id, _) in top_inflow1
                push!(xL, x_coords[cell_id])
                push!(uL, num_sol[cell_id])
            end
            for (cell_id, _) in top_inflow2
                push!(xR, x_coords[cell_id])
                push!(uR, num_sol[cell_id])
            end
        end
...
function get_Pnorm_for_finalTimeInterface(grid)
    ...
    for (cell_id, face_id) in TOP_INFLOW
        ...
        # Pnorm[cell_id] = (0.0277777777777777 / 2) * ref.H_face
