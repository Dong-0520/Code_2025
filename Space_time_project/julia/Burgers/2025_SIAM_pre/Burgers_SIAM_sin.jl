...
grid = read_mesh(joinpath(@__DIR__, "Burgers_SIAM.msh"), ref_elems_data, Base.identity)
@save joinpath(@__DIR__, "Burgers_SIAM_sin_mesh.jld2") grid
@load joinpath(@__DIR__, "Burgers_SIAM_sin_mesh.jld2") grid
...
@time all_U, diffs = space_time_solve(U0, p1, pseudo_dt = 0.001, num_of_pseudo_time_step = 2000, diss = true, test = true)
plot(diffs)
...
# 求解问题
sol_slab3 = solve(prob_with_jac_diss, NewtonRaphson(),show_trace = Val(true), maxiters = 1000)
u3 = sol_slab3.u
...
nonlinearFunction_with_jac_diss = NonlinearFunction(RHS_forSolver_diss; jac_prototype=jac_sparsity_diss)
prob_with_jac_diss = NonlinearProblem(nonlinearFunction_with_jac_diss, U0_for_solver, p4; abstol=1e-10, reltol=1e-10)

sol_slab4 = solve(prob_with_jac_diss, NewtonRaphson(),show_trace = Val(true), maxiters = 1000)
u4 = sol_slab4.u


#---------------------- slab 5 ---------------------- #
p5 = get_set_up(grid, u = u4, slab = 5)
all_U5, diffs5 = space_time_solve(u4, p5, pseudo_dt = 0.00005, num_of_pseudo_time_step = 5000, diss = true, test = true)
...
nonlinearFunction_with_jac_diss = NonlinearFunction(RHS_forSolver_diss; jac_prototype=jac_sparsity_diss)
prob_with_jac_diss = NonlinearProblem(nonlinearFunction_with_jac_diss, U0_for_solver, p5; abstol=1e-10, reltol=1e-10)
...
all_U6, diffs6 = space_time_solve(u5, p6, pseudo_dt = 0.0001, num_of_pseudo_time_step = 1000, diss = true, test = true)
...
nonlinearFunction_with_jac_diss = NonlinearFunction(RHS_forSolver_diss; jac_prototype=jac_sparsity_diss)
prob_with_jac_diss = NonlinearProblem(nonlinearFunction_with_jac_diss, U0_for_solver, p6; abstol=1e-10, reltol=1e-10)
...
u6 = all_U7[argmin(diffs7)]
all_U7, diffs7 = space_time_solve(u6, p7, pseudo_dt = 0.00005, num_of_pseudo_time_step = 1000, diss = true, test = true)
argmin(diffs7)
U0_for_solver = all_U7[argmin(diffs7)]
observe_numerical_sol(grid, U0_for_solver, zlimit, show_true_sol = false)

gif(observe_iteration(grid, all_U7[1:10:end, :, :], zlimit, show_true_sol = false), joinpath(@__DIR__, "numerical_test_discts.gif"), fps=5)
observe_numerical_sol(grid, all_U7[325], zlimit, show_true_sol = false)


U0_for_solver = all_U7[325]
...
nonlinearFunction_with_jac_diss = NonlinearFunction(RHS_forSolver_diss; jac_prototype=jac_sparsity_diss)
prob_with_jac_diss = NonlinearProblem(nonlinearFunction_with_jac_diss, U0_for_solver, p7; abstol=1e-10, reltol=1e-10)

sol_slab7 = solve(prob_with_jac_diss, NewtonRaphson(),show_trace = Val(true), maxiters = 200)
u7 = sol_slab7.u

#---------------------- slab 8 ---------------------- #
p8 = get_set_up(grid, u = u7, slab = 8)
all_U8, diffs8 = space_time_solve(u7, p8, pseudo_dt = 0.00005, num_of_pseudo_time_step = 2000, diss = true, test = true)
...
nonlinearFunction_with_jac_diss = NonlinearFunction(RHS_forSolver_diss; jac_prototype=jac_sparsity_diss)
prob_with_jac_diss = NonlinearProblem(nonlinearFunction_with_jac_diss, U0_for_solver, p8; abstol=1e-10, reltol=1e-10)

sol_slab8 = solve(prob_with_jac_diss, NewtonRaphson(),show_trace = Val(true), maxiters = 1000)
u8 = sol_slab8.u
...
U0_for_solver = all_U9[argmin(diffs9)]
observe_numerical_sol(grid, U0_for_solver, zlimit, show_true_sol = false)
...
nonlinearFunction_with_jac_diss = NonlinearFunction(RHS_forSolver_diss; jac_prototype=jac_sparsity_diss)
prob_with_jac_diss = NonlinearProblem(nonlinearFunction_with_jac_diss, U0_for_solver, p9; abstol=1e-10, reltol=1e-10)

sol_slab9 = solve(prob_with_jac_diss, NewtonRaphson(),show_trace = Val(true), maxiters = 1000)
u9 = sol_slab9.u
...
nonlinearFunction_with_jac_diss = NonlinearFunction(RHS_forSolver_diss; jac_prototype=jac_sparsity_diss)
prob_with_jac_diss = NonlinearProblem(nonlinearFunction_with_jac_diss, U0_for_solver, p10; abstol=1e-10, reltol=1e-10)

sol_slab10 = solve(prob_with_jac_diss, NewtonRaphson(),show_trace = Val(true), maxiters = 1000)
u10 = sol_slab10.u
...
# check BurgerMesh_SIAM file for dt
scatter_gif_numerical_sol_for_all_slabs(grid, [u1, u2, u3, u4, u5, u6, u7, u8, u9, u10], 0.025, color = :blue, fps = 2 )

plot_gif_numerical_sol_for_all_slabs(grid, [u1, u2, u3, u4, u5, u6, u7, u8, u9, u10], 0.025, color = :blue, fps = 2 )


plot_numerical_sol_at_final_time_perSlab(grid, u5, color = :red)



bottom_inflow = collect(union(get_face_set(grid, "BOTTOM_INFLOW_1"), get_face_set(grid, "BOTTOM_INFLOW_2")))
p = plot(title=@sprintf("Time = %.3f", 0.0), xlabel="x", ylabel="u", ylim=[-1.1, 1.1], legend=false, grid = false)
x = []
u = []
for (cell_id, face_id) in bottom_inflow
    ref = grid.cells[cell_id].ref_data[]
...
x_sorted = x_flat[sorted_indices]
u_sorted = u_flat[sorted_indices]
plot!(p, x_sorted, u_sorted, label="", color=:blue)
scatter!(p, x_sorted, u_sorted, shape=:utriangle, markersize=3, color=:red)
save(joinpath(@__DIR__, "plots_sin/Burger_sin_1.png"), p)
save(joinpath(@__DIR__, "plots_sin/Burger_sin_2.png"), plot_numerical_sol_at_final_time_perSlab(grid, u1, title = 0.025, color=:blue))
save(joinpath(@__DIR__, "plots_sin/Burger_sin_3.png"), plot_numerical_sol_at_final_time_perSlab(grid, u2, title = 0.05, color=:blue))
save(joinpath(@__DIR__, "plots_sin/Burger_sin_4.png"), plot_numerical_sol_at_final_time_perSlab(grid, u3, title = 0.075, color=:blue))
save(joinpath(@__DIR__, "plots_sin/Burger_sin_5.png"), plot_numerical_sol_at_final_time_perSlab(grid, u4, title = 0.1, color=:blue))
save(joinpath(@__DIR__, "plots_sin/Burger_sin_6.png"), plot_numerical_sol_at_final_time_perSlab(grid, u5, title = 0.125, color=:blue))
save(joinpath(@__DIR__, "plots_sin/Burger_sin_7.png"), plot_numerical_sol_at_final_time_perSlab(grid, u6, title = 0.15, color=:blue))
save(joinpath(@__DIR__, "plots_sin/Burger_sin_8.png"), plot_numerical_sol_at_final_time_perSlab(grid, u7, title = 0.175, color=:blue))
save(joinpath(@__DIR__, "plots_sin/Burger_sin_9.png"), plot_numerical_sol_at_final_time_perSlab(grid, u8, title = 0.2, color=:blue))
save(joinpath(@__DIR__, "plots_sin/Burger_sin_10.png"), plot_numerical_sol_at_final_time_perSlab(grid, u9, title = 0.225, color=:blue))
save(joinpath(@__DIR__, "plots_sin/Burger_sin_11.png"), plot_numerical_sol_at_final_time_perSlab(grid, u10, title = 0.25, color=:blue))


save(joinpath(@__DIR__, "plots_sin/Burger_sin_mesh.png"), plot_mesh(grid, xlimit=[0, 0.5], ylimit = [0, 0.025]))

observe_numerical_sol(grid, u1, zlimit, show_true_sol = false)
observe_numerical_sol(grid, u2, zlimit, show_true_sol = false)
observe_numerical_sol(grid, u3, zlimit, show_true_sol = false)
observe_numerical_sol(grid, u4, zlimit, show_true_sol = false)
observe_numerical_sol(grid, u5, zlimit, show_true_sol = false)
observe_numerical_sol(grid, u6, zlimit, show_true_sol = false)
observe_numerical_sol(grid, u7, zlimit, show_true_sol = false)
observe_numerical_sol(grid, u8, zlimit, show_true_sol = false)
observe_numerical_sol(grid, u9, zlimit, show_true_sol = false)
observe_numerical_sol(grid, u10, zlimit, show_true_sol = false)



# ---------------------- develop plotting style ---------------------- #
order = 3

ref = TriangleDiagELG(order, 2 * order)
ref_elems_data = Dict{String, SBPLite.RefElemData}("Triangle 3" => ref)

grid = read_mesh(joinpath(@__DIR__, "mesh_example.msh"), ref_elems_data, Base.identity)

using Measures
function plot_mesh(grid; xlimit=[0, 1], ylimit=[0, 0.1], size = (800, 600))
    num_of_cells = n_cells(grid)
    num_of_nodes = Int(length(grid.xyz_q[1]))
    # xlim = [xlimit[1]-0.01, xlimit[2]+0.01]
    # ylim = [ylimit[1]-0.01, ylimit[2]+0.01]
    xlim = xlimit
    ylim = ylimit
    p = scatter(xticks=[xlimit[1], xlimit[2]], 
                                        yticks=[], 
                                        xlimit=xlim, ylimit=ylim,
                                         legend=false, grid=false, size=size, framestyle=:none)
    # annotate!(xlim[1]-0.0075, (ylim[1]+ylim[2])/2, text("t", 10, :black))
    # annotate!((xlim[1]+xlim[2])/2, ylim[1]-0.005, text("x", 10, :black))
    ...
    for cell_id in 1:num_of_cells
        ...
        plot!(p, [v1x, v2x, v3x, v1x], [v1y, v2y, v3y, v1y], label="", color=:blue, linewidth=0.3)
...
save(joinpath(@__DIR__, "spacetime_slab2.png"),plot_mesh(grid, xlimit=[0, 1], ylimit = [0, 0.1], size = (1000, 200)))


# check if the average flux is on

function rankine_hugoniot_test(u, p)
    grid, interfaces_align_shock, interior_interfaces, IC = p
    interior_interfaces = collect(interior_interfaces)
    result = zeros(n_cells(grid), nodes_per_cell)

    for i in 1:length(interior_interfaces)
        interface = interior_interfaces[i]
        ck, lfγk = interface.face_1
        cv, lfγv = interface.face_2
        Pk, Pv = interface.P1, interface.P2
        ...
        indicator = Nxγk .* (uk_face - uk_adj)
        if all( indicator .>= 0 )
            println("average flux is on for ", ck, " ", cv)
        end
        # spatial_flux_k = (uk_face.^2 + uk_adj.^2) ./ 4 .* (indicator .>= 0) + ((uk_face.^2 + uk_face .* uk_adj + uk_adj.^2) ./ 6 .+ mysign.(Nxγk) .* max.(abs.(uk_face), abs.(uk_adj)) .* (uk_face .- uk_adj)) .* (indicator .< 0)
...
rankine_hugoniot_test(u4, p4)
rankine_hugoniot_test(u5, p5)