...
grid = read_mesh(joinpath(@__DIR__, "Burgers_sin_3.msh"), ref_elems_data, Base.identity)


@save joinpath(@__DIR__, "Burgers_sin_1_mesh.jld2") grid
@load joinpath(@__DIR__, "Burgers_sin_1_mesh.jld2") grid

@save joinpath(@__DIR__, "Burgers_sin_2_mesh.jld2") grid
@load joinpath(@__DIR__, "Burgers_sin_2_mesh.jld2") grid

@save joinpath(@__DIR__, "Burgers_sin_3_mesh.jld2") grid
@load joinpath(@__DIR__, "Burgers_sin_3_mesh.jld2") grid

@save joinpath(@__DIR__, "Burgers_sin_4_mesh.jld2") grid
@load joinpath(@__DIR__, "Burgers_sin_4_mesh.jld2") grid
...
@time all_U, diffs = space_time_solve(U0, p1, pseudo_dt = 0.0005, num_of_pseudo_time_step = 2000, diss = true, test = true)
plot(diffs)
...
# 求解问题
sol_slab2 = solve(prob_with_jac_diss, NewtonRaphson(),show_trace = Val(true), maxiters = 200)
...
# 求解问题
sol_slab3 = solve(prob_with_jac_diss, NewtonRaphson(),show_trace = Val(true), maxiters = 200)
u3 = sol_slab3.u
...
sol_slab4 = solve(prob_with_jac_diss, NewtonRaphson(),show_trace = Val(true), maxiters = 200)
u4 = sol_slab4.u
...
function plot_dict_u(analytic_u, x_coords)
    """
    given a dictionary of u and x_coords with the same keys, plot the u at each cell
    """
    p = scatter()
    for cell_id in collect(keys(analytic_u))
        scatter!(p, x_coords[cell_id], analytic_u[cell_id], label = "", color = "red")
    end
    return p
end

x_coords = get_xCoords_for_plotting_sol(grid)
u = solve_u(x_coords, 0.1)
num_u = get_final_u_for_plotting_sol(grid, u4)
Pnorms = get_Pnorm_for_finalTimeInterface(grid)

plot_dict_u(u, x_coords)
plot_dict_u(num_u, x_coords)
error_u = Dict()
for k in keys(u)
...
@save joinpath(@__DIR__, "numerical_sol/numerical_solutions_mesh1.jld2") grid u1 u2 u3 u4 L2_error
@save joinpath(@__DIR__, "numerical_sol/numerical_solutions_mesh2.jld2") grid u1 u2 u3 u4 L2_error


# mesh 1 -> L2_error = 0.0001764249107080106
# mesh 2 -> L2_error = 0.0009920705323272856
# mesh 3 -> L2_error = 0.0003856762663942843
# mesh 4 -> L2_error = 0.002646972176892316
...
@load joinpath(@__DIR__, "mesh_data/Burgers_sin_mesh1_oder2.jld2") grid
grid1 = grid

@load joinpath(@__DIR__, "mesh_data/Burgers_sin_mesh2_oder2.jld2") grid
grid2 = grid

@load joinpath(@__DIR__, "mesh_data/Burgers_sin_mesh3_oder2.jld2") grid
grid3 = grid

@load joinpath(@__DIR__, "mesh_data/Burgers_sin_mesh4_oder2.jld2") grid
grid4 = grid

dxs_ave_length = [get_average_length_of_sides(grid1), get_average_length_of_sides(grid2), get_average_length_of_sides(grid4)]
errors = [0.0001764249107080106, 0.0009920705323272856, 0.002646972176892316]
fit(log10.(dxs_ave_length[1:end]), log10.(errors[1:end]),1)[1]


# this is data for order = 2
# grid 1, 2, 4 are recorded, 3 is weired, so we only use 1, 2, 4 to compute the convergence rate
# dxs_ave_length = [0.009639741840102562, 0.016621299438028026, 0.024650661447302023]
# errors = [0.0001764249107080106, 0.0009920705323272856, 0.002646972176892316]
# slope = 2.902054118685605