...
function get_interior_interfaces_periodicBC(grid)
    ...
    for interface in grid.face_interfaces
        ...
        if all(x -> isapprox(x, 0, atol = 1e-12), ck_face_x .- cv_face_x) && all(x -> isapprox(x, 0, atol = 1e-12), ck_face_y .- cv_face_y)
...
function RHS_test(u, p)
    ...
    #     result[ck, :] += rk_spatial
    #     result[cv, :] += rv_spatial
    # end
...
#---------------------- debug ---------------------- #
# FaceInterface(FaceIndex((16, 1)), FaceIndex((2, 1)), [1, 2, 3, 4], [1, 2, 3, 4])
# FaceInterface(FaceIndex((3, 4)), FaceIndex((13, 4)), [1, 2, 3, 4], [1, 2, 3, 4])
...
ux16 = 1/3 * Dx16 * (u16 .^ 2) .+ 1/3 * u16 .* (Dx16 * u16)
ut16 = Dt16 * u16
ux2 = 1/3 * Dx2 * (u2 .^ 2) .+ 1/3 * u2 .* (Dx2 * u2)
ut2 = Dt2 * u2
du16 = -ux16 .- ut16
du2 = -ux2 .- ut2


interface = FaceInterface(FaceIndex((16, 1)), FaceIndex((2, 1)), [1, 2, 3, 4], [1, 2, 3, 4])
ck, lfγk = interface.face_1
cv, lfγv = interface.face_2
# Pk, Pv = interface.P1, interface.P2
refk = grid.cells[ck].ref_data[]
...
du16 += rk_spatial .+ rk_temporal
du2 += rv_spatial .+ rv_temporal
