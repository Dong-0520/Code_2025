...
function Chandrashekar1D_f1_forSolver(ρki::T, ρvj::T, uki::T, uvj::T) where T<:Real
...
function Chandrashekar1D_f2_forSolver(ρki::T, ρvj::T, uki::T, uvj::T, βki::T, βvj::T) where T<:Real
    return middle(uki, uvj) * Chandrashekar1D_f1_forSolver(ρki, ρvj, uki, uvj) + (middle(ρki, ρvj) / (2 * middle(βki, βvj)))
end

function Chandrashekar1D_f3_forSolver(ρki::T, ρvj::T, uki::T, uvj::T, βki::T, βvj::T; γ = 1.4) where T<:Real
...
function two_point_flux_forSolver(Wki::AbstractArray{<:Real, 1}, Wvj::AbstractArray{<:Real, 1}; γ = 1.4)
    """
...
function two_point_flux_function_forSolver(Wk::AbstractArray{<:Real, 2}, Wv::AbstractArray{<:Real, 2}; γ = 1.4)
    """
...
function two_point_state_flux_forSolver(Wki::AbstractArray{<:Real, 1}, Wvj::AbstractArray{<:Real, 1}; γ = 1.4, R = 8.31446261815324)
    # 提取变量
    ρki, ρvj = Wki[1], Wvj[1]
    uki, uvj = Wki[2], Wvj[2]
    pki, pvj = Wki[3], Wvj[3]
...
function two_point_state_flux_function_forSolver(Wk::AbstractArray{<:Real, 2}, Wv::AbstractArray{<:Real, 2}; γ = 1.4, R = 8.31446261815324)
    """
    two argument matrix function
    Wk, Wv: 2-dimensional arrays of primitive variables (density, velocity, pressure) (ρ, u, p)
    say there are m variables and N nodes per element
