...
# grid = read_mesh(joinpath(@__DIR__, "Burgers_sin_coarse_mesh.msh"), ref_elems_data, identity)
# @save joinpath(@__DIR__, "Burgers_sin_coarse_mesh.jld2") grid
@load joinpath(@__DIR__, "Burgers_sin_coarse_mesh.jld2") grid
# ax, at = 1.0, 1.0
# bottomBC1(x::Coord) = -1
# bottomBC2(x::Coord) = 1

analytic_u(x::Coord) = sinpi(4 * x[1])
...
# --------------------------------------------------------------------
# solve the burgers equation using implicit function at t = 0.1
using Roots
...
for (cell_id, face_id) in BOTTOM_INFLOW
    ...
    x_coords[cell_id] = map(x -> x[1], R * grid.xyz_q[cell_id])
end

bottom_bc_before_opt = solve_u(x_coords, 0.1)
...
function RHS(du, u, p)
    ...
    @inbounds Threads.@threads for i in 1:length(left_inflow)
        ...
        u_face = R * u[cell_id, :]
        u_adj = zeros(Float64, length(u_face))  # left boundary condition is zero
    ...
    @inbounds Threads.@threads for i in 1:length(right_inflow)
        ...
        u_face = R * u[cell_id, :]
        u_adj = zeros(Float64, length(u_face))  # right boundary condition is zero
...
U0_for_solver = initial_guess(grid)
...
para = grid, bottom_bc_before_opt
...
plot_mesh(grid)
@save joinpath(@__DIR__, "oscillating_solution_sin_coarse.jld2") oscillating_u grid


using JuMP, MadNLP, Random, MadNLPHSL
@load joinpath(@__DIR__, "oscillating_solution_sin_coarse.jld2") oscillating_u grid

include(joinpath(@__DIR__, "../../src_simpleGrid/SimpleGrid.jl"))

function RHS_for_solution(du, u, para)
    grid, a, bottom_bc = para
    ref = grid.ref
    ...
    for (cell_id, face_id) in union(grid.face_sets["BOTTOM_INFLOW_1"], grid.face_sets["BOTTOM_INFLOW_2"])
        ...
        u_face = R * u[cell_id, :]
        # u_adj = bottom_bc[cell_id]
        x_coords = map(x -> x[1], R * grid.xyz_SBP[cell_id])
...
function wrap_up_results(buffer; num_of_nodes = length(grid.xyz_gmsh))
    ...
    new_xyz_gmsh_coords = [Coord{2, Float64}((new_xyz_gmsh[i], new_xyz_gmsh[buffer.index_mesh ÷ 2 + i])) for i in 1:num_of_nodes]
...
function pde_model_with_solver_struct(buffer::SpaceTimeBuffer, bottom_bc::Dict; lin_solver = MadNLPHSL.Ma57Solver, 
    ...
    function pde_residual(x::T...) where {T<:Real}
        ...
        new_para = (current_grid, buffer.a, bottom_bc)
...
model1 = pde_model_with_solver_struct(buffer, Dict(), αmesh = 5, αshk = 10000, max_iter = 200, lin_solver = MadNLPHSL.Ma27Solver)
...
@load joinpath(@__DIR__, "../optimization_results//buffer_two_coordinates_sin_problem_005.jld2") buffer final_simple_grid final_u
...
bottom_bc_before_opt = solve_u(x_coords_initial_before_opt, 0.1)
...
top_bc_before_opt = solve_u(x_coords_final_before_opt, 0.15)
...
title!(final_solution_comparision_before_opt, "Numerical vs Analytic at t = 0.05 \n before optimization")
xlabel!(final_solution_comparision_before_opt, "x")
ylabel!(final_solution_comparision_before_opt, "u")
savefig(final_solution_comparision_before_opt, joinpath(@__DIR__, "../plots_sin_burgers/burgers_final_sol_compare_before_opt_sin_problem.png"))
...
savefig(initial_solution_comparision_before_opt, joinpath(@__DIR__, "../plots_sin_burgers/burgers_initial_sol_compare_before_opt_sin_problem.png"))
...
top_bc_after_opt = solve_u(x_coords_final_after_opt, 0.15)
...
bottom_bc_after_opt = solve_u(x_coords_initial_after_opt, 0.1)
...
savefig(initial_solution_comparision_after_opt, joinpath(@__DIR__, "../plots_sin_burgers/burgers_initial_sol_compare_after_opt_sin_problem.png"))
...
title!(final_solution_comparision_after_opt, "Numerical vs Analytic at t = 0.05 \n after optimization")
xlabel!(final_solution_comparision_after_opt, "x")
ylabel!(final_solution_comparision_after_opt, "u")
savefig(final_solution_comparision_after_opt, joinpath(@__DIR__, "../plots_sin_burgers/burgers_final_sol_compare_after_opt_sin_problem.png"))
...
plot!(mesh_comparision, [0.25, 0.25], [0.0, 0.05], label = "True shock", color = :red, markersize = 5)
mesh_comparision
savefig(mesh_comparision, joinpath(@__DIR__, "../plots_sin_burgers/burgers_mesh_comparision_sin_problem.png"))
...
scatter!(mesh_nonaligned, 
          [-0.1, 0.1], [0.0, 0.2], 
          label = "", color = :red, markersize = 5)  # 添加蓝色标签的占位符
plot!(mesh_nonaligned, [0.25, 0.25], [0.0, 0.05], label = "True shock", color = :red)
savefig(mesh_nonaligned, joinpath(@__DIR__, "../plots_sin_burgers/burgers_mesh_nonaligned_sin_problem.png"))
...
scatter!(mesh_after_opt, 
          [-0.1, 0.1], [0.0, 0.2], 
          label = "", color = :red, markersize = 5)  # 添加蓝色标签的占位符
plot!(mesh_after_opt, [0.25, 0.25], [0.0, 0.05], label = "True shock", color = :red, markersize = 5)
mesh_after_opt
savefig(mesh_after_opt, joinpath(@__DIR__, "../plots_sin_burgers/burgers_mesh_after_opt_sin_problem.png"))

gr()
error_comparison_final = scatter(grid = false, xlabel = "x", ylabel = "Error", title = "Error Comparison at t = 0.05")
first_blue = true
first_red = true
for (cell_id, face_id) in TOP_INFLOW
...
savefig(error_comparison_final, joinpath(@__DIR__, "../plots_sin_burgers/burgers_error_comparison_final_sin_problem.png"))
...
savefig(error_comparison_initial, joinpath(@__DIR__, "../plots_sin_burgers/burgers_error_comparison_initial_sin_problem.png"))
