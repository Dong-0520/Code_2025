...
grid = read_mesh(joinpath(@__DIR__, "../Burgers_sin_mesh2_2_005.msh"), ref_elems_data, identity)
...
function solve_u(xs_dict, t; u_init::Float64 = 0.0)
    U = Dict()  
...
function wrap_up_results(buffer; num_of_element = n_cells(grid))
    ...
    new_xyz_gmsh_coords = [Coord{2, Float64}((new_xyz_gmsh[i], new_xyz_gmsh[buffer.index_mesh ÷ 2 + i])) for i in 1:num_of_element]
...
model1 = pde_model_with_solver_struct(buffer, u, αmesh = 10, αshk = 100000, max_iter = 100, lin_solver = MadNLPHSL.Ma27Solver)
