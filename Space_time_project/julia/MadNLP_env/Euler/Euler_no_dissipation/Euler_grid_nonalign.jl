...
name = "Euler_grid_$(meshNumber)_nonalign"
gmsh.model.add(name)
...
S1 = one_wave_speed(ρL, uL, pL, pstar) - 0.75
S2 = ustar
S3 = three_wave_speed(ρR, uR, pR, pstar) + 0.75
...
# 设置底边的节点数量
gmsh.model.mesh.setTransfiniteCurve(1, 3)  # 左底边：3个节点
gmsh.model.mesh.setTransfiniteCurve(5, 3)  # 右底边：3个节点

# 设置顶边的节点数量
gmsh.model.mesh.setTransfiniteCurve(3, 3)  # 左顶边：3个节点
gmsh.model.mesh.setTransfiniteCurve(7, 3)  # 右顶边：3个节点

# 设置垂直边和斜边的节点数量
gmsh.model.mesh.setTransfiniteCurve(4, 2)  # 左边：2个节点
gmsh.model.mesh.setTransfiniteCurve(8, 2)  # 右边：2个节点
