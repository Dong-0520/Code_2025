...
function create_new_grid(grid::Grid{dim, C, T}; 
    ...
    for c in grid.cells
        ...
        push!(curvilinear_mapping, PolynomialCurvilinearMapping{dim, T, basis}(coeffs))
    ...
    return Grid(deepcopy(grid.cells), new_xyz_gmsh, curvilinear_mapping, deepcopy(grid.ref_elems_data),
                ...
                vector_of_nodeTags = Vector{Vector{Int64}}(),
                vector_of_nodeTagsMaster = Vector{Vector{Int64}}(),
                is_periodic = false)
end
...
using Test
function polynomial_exactness_test(grid, degree::Int = 1) where {dim, C, T}
    @assert degree >= 1
    for cell_id in 1:n_cells(grid)
        ...
        @test all(abs.(Dx * p1 .- dp1dx) .< 1e-8)
        @test all(abs.(Dt * p1 .- dp1dt) .< 1e-8)

        p2 = coeff1 .* x .^ degree + coeff2 .* t .^ degree
        dp2dx = coeff1 .* degree .* x .^ (degree - 1)
        dp2dt = coeff2 .* degree .* t .^ (degree - 1)
        @test all(abs.(Dx * p2 .- dp2dx) .< 1e-8)
        @test all(abs.(Dt * p2 .- dp2dt) .< 1e-8)

    end
end

polynomial_exactness_test(newgrid, 2)