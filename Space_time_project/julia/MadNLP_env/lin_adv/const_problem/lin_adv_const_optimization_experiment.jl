...
buffer.indices_moving_coords = [2, 10]
buffer.indices_moving_coords
model1 = pde_model_with_solver_struct(buffer, αmesh = 0, αshk = 10, max_iter = 50, lin_solver = MadNLPHSL.Ma57Solver)
...
TOP_INFLOW = [FaceIndex((9, 1)), FaceIndex((13, 1)), FaceIndex((10, 1)), FaceIndex((8, 1))]
...
final_solution_comparision_before_opt = plot(xlims = (-1.01, 1.01), grid = false)
first_blue = true
first_red = true
for (cell_id, face_id) in TOP_INFLOW
...
final_solution_comparision_after_opt = plot(xlims = (-1.01, 1.01), grid = false)
first_blue = true
first_red = true
for (cell_id, face_id) in TOP_INFLOW
...
savefig(final_solution_comparision_after_opt, joinpath(@__DIR__, "../plots_const/linadv_final_sol_compare_after_opt_const_problem.png"))
...
mesh_comparision
savefig(mesh_comparision, joinpath(@__DIR__, "../plots_const/linadv_mesh_comparision_const_problem.png"))

mesh_nonaligned = scatter(xlabel = "x", ylabel = "t", title = "Mesh comparison (Non-aligned)", 
                                    xlims = xlimit, ylims = ylimit, size = plot_size, 
                                    margin = 5mm, legend = (0.5, -0.25), grid = false, bottom_margin = 20mm)
first_blue = true
# 画网格
...
savefig(mesh_nonaligned, joinpath(@__DIR__, "../plots_const/linadv_mesh_nonaligned_const_problem.png")) 
...
mesh_after_opt
savefig(mesh_after_opt, joinpath(@__DIR__, "../plots_const/linadv_mesh_after_opt_const_problem.png"))
