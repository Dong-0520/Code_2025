...
grid = read_mesh(joinpath(@__DIR__, "lin_adv_grid_nonalign_6.msh"), ref_elems_data, identity)
@save joinpath(@__DIR__, "lin_adv_grid_nonalign_6.jld2") grid
@load joinpath(@__DIR__, "lin_adv_grid_nonalign_6.jld2") grid
ax, at = 1.0, 1.0
# bottomBC1(x::Coord) = -1
# bottomBC2(x::Coord) = 1
...
function pde_model_with_solver_struct(buffer::SpaceTimeBuffer; lin_solver = MadNLPHSL.Ma57Solver, 
    ...
    model = Model(() -> MadNLP.Optimizer(
        ...
        # 🔥 更严格的收敛条件
        tol=1e-20,                          # 主要容差（从1e-8改为1e-12）
        
        # 🔥 禁用可接受解机制，强制达到严格收敛
        acceptable_tol=1e-20,               # 设置比主容差更严格
    ...
    function mesh_shock_objective(x::T...) where {T<:Real}
        ...
        for cell_id in 1:num_of_element
            ...
            if cell_id in cells_aligning_shock
                r_shk[cell_id] = αshk2 * sum((num_uk .- num_̄uk).^2 .* quadrature_k) / area_K  # 计算 shock tracking residual
            else
...
buffer.indices_moving_coords = [2, 5]
buffer.indices_moving_coords
model1 = pde_model_with_solver_struct(buffer, αmesh = 10, αshk = 2000, max_iter = 50, lin_solver = MadNLPHSL.Ma57Solver, cells_aligning_shock = [], αshk2 = 2)

# using Plots
# include(joinpath(@__DIR__, "../plot_helper_simple_grid.jl"))
final_simple_grid, final_u = wrap_up_results(buffer; num_of_cells = n_cells(grid))
plot_u_interactive(final_simple_grid, final_u)
plot_mesh(final_simple_grid)

@save joinpath(@__DIR__, "../optimization_results//buffer_two_coordinates_sin_problem_finer_mesh.jld2") buffer final_simple_grid final_u




@load joinpath(@__DIR__, "../optimization_results//buffer_two_coordinates_sin_problem_finer_mesh.jld2") buffer final_simple_grid final_u
...
savefig(final_solution_comparision_before_opt, joinpath(@__DIR__, "../plots_sin_larger1/linadv_final_sol_compare_before_opt_sin_problem.png"))
...
savefig(initial_solution_comparision_before_opt, joinpath(@__DIR__, "../plots_sin_larger1/linadv_initial_sol_compare_before_opt_sin_problem.png"))
...
savefig(initial_solution_comparision_after_opt, joinpath(@__DIR__, "../plots_sin_larger1/linadv_initial_sol_compare_after_opt_sin_problem.png"))
...
savefig(final_solution_comparision_after_opt, joinpath(@__DIR__, "../plots_sin_larger1/linadv_final_sol_compare_after_opt_sin_problem.png"))
...
mesh_comparision
savefig(mesh_comparision, joinpath(@__DIR__, "../plots_sin_larger1/linadv_mesh_comparision_sin_problem.png"))
...
mesh_nonaligned
savefig(mesh_nonaligned, joinpath(@__DIR__, "../plots_sin_larger1/linadv_mesh_nonaligned_sin_problem.png"))
...
mesh_after_opt
savefig(mesh_after_opt, joinpath(@__DIR__, "../plots_sin_larger1/linadv_mesh_after_opt_sin_problem.png"))
...
savefig(error_comparison_final, joinpath(@__DIR__, "../plots_sin_larger1/linadv_error_comparision_final_sin_problem.png"))
...
savefig(error_comparison_initial, joinpath(@__DIR__, "../plots_sin_larger1/linadv_error_comparision_initial_sin_problem.png"))
